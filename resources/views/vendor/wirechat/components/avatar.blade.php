...
<div
    ...
    @if (!$src && $group)
        ...
        <svg class="h-full w-full shrink-0 scale-90 rounded-full bg-gray-100 text-gray-300 dark:bg-gray-600"
    ...
    @elseif(!$src)
        <svg class="h-full w-full shrink-0 rounded-full" fill="currentColor" viewBox="0 0 24 24">
            <path
    ...
    @if ($disappearing)
        <span dusk="disappearing_messages_icon"
            class="absolute -bottom-1 -right-2 z-50 rounded-full bg-white p-px dark:bg-gray-800">
            <svg class="h-5 w-5" viewBox="0 0 36 36" height="36" width="36" preserveAspectRatio="xMidYMid meet"
                fill="none">
